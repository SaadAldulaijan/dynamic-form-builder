...
export class DropdownControl {
  readonly field = input.required<DropdownFieldSchema>();
  ...
  readonly controlClass = computed(() => {
    return this.field().layout?.controlClass ?? 'form-select';
  });
  ...
  readonly options = computed(() => {
    ...
    return (currentField.options ?? []).filter(option => option.parentValue === parentValue);
  });
